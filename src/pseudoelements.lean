...
begin
  ...
  { rintro ⟨R, p, q, ep, eq, comm⟩,
    apply (@preadditive.cancel_zero_iff_epi _ _ _ _ _ p).1 ep _ f.snd,
    rw comm,
...
begin
  intro h,
  apply preadditive.cancel_zero_iff_mono.2,
  intros R g hg,
...
begin
  ...
  { intro b',
    ...
    rw category.id_comp,
    conv_lhs { congr, skip, rw ←image.fac f, },
    rw ←category.assoc,
    rw pullback.condition,
    rw category.assoc,
    congr,
...
begin
  ...
  { set i := kernel.ι (cokernel.π f),
    let p := factor_thru_image f,
    have gip : p ≫ i ≫ g = 0,
    { rw ←category.assoc, rw image.fac f, exact this },
    have gi : i ≫ g = 0,
    have e : epi (factor_thru_image f), by apply_instance,
    { apply preadditive.cancel_zero_iff_epi.1 e _ _, exact gip, },
    let b := kernel.ι g,
...
begin
  ...
  { erw pseudo_apply_calc,
    ...
    erw preadditive.distrib_left,
    rw preadditive.neg_left,
    simp only [category.assoc],
    rw ←sub_eq_add_neg,
  ...
  { intros Z g hh,
    ...
    have st := preadditive.cancel_zero_iff_epi.1 ep' _ (a'.snd ≫ g) comm',
    apply quotient.sound,
    use R,
    use 𝟙 R,
    use p,
